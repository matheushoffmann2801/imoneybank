    ...
    return (
        <div className="fixed inset-0 bg-slate-900 text-white flex flex-col font-sans">
            ...
            <div className="bg-slate-800 p-4 shadow-lg border-b border-slate-700 flex justify-between items-center z-10">
                ...
                <button onClick={() => { play('click'); onLogout(); }} className="bg-red-500/20 text-red-400 p-2 rounded-lg hover:bg-red-500 hover:text-white transition">
                    <LogOut size={20} />
                </button>
            </div>
            ...
            <div className="bg-slate-800/50 p-3 flex gap-2 overflow-x-auto border-b border-slate-700">
                <button onClick={() => { play('click'); setActiveTab('players'); }} className={`px-4 py-2 rounded-lg font-bold text-xs flex items-center gap-2 shadow-md transition ${activeTab === 'players' ? 'bg-slate-700 text-white border border-slate-500' : 'bg-slate-800 text-slate-400'}`}>
                    <Users size={16} /> JOGADORES
                </button>
                <button onClick={() => { play('click'); setActiveTab('logs'); }} className={`px-4 py-2 rounded-lg font-bold text-xs flex items-center gap-2 shadow-md transition ${activeTab === 'logs' ? 'bg-slate-700 text-white border border-slate-500' : 'bg-slate-800 text-slate-400'}`}>
                    <ClipboardList size={16} /> LOGS
                </button>
                <div className="w-px h-8 bg-slate-700 mx-2"></div>
                <button onClick={() => { play('click'); executeAction('pass_turn'); }} className="bg-blue-600 text-white px-4 py-2 rounded-lg font-bold text-xs flex items-center gap-2 shadow-md active:scale-95 whitespace-nowrap">
                    <SkipForward size={16} /> PASSAR VEZ
                </button>
                <button onClick={() => { play('click'); onDrawCard(); }} className="bg-purple-600 text-white px-4 py-2 rounded-lg font-bold text-xs flex items-center gap-2 shadow-md active:scale-95 whitespace-nowrap">
                    <Sparkles size={16} /> SORTE/REVÉS
                </button>
                <button onClick={() => { play('click'); if(confirm("ATENÇÃO: Isso destruirá 1 casa de CADA jogador que possuir construções. Continuar?")) executeAction('global_earthquake'); }} className="bg-orange-700 text-white px-4 py-2 rounded-lg font-bold text-xs flex items-center gap-2 shadow-md active:scale-95 whitespace-nowrap border border-orange-600">
                    <Activity size={16} /> TERREMOTO
                </button>
                <button onClick={() => { play('click'); onCloseRoom(); }} className="bg-red-600 text-white px-4 py-2 rounded-lg font-bold text-xs flex items-center gap-2 shadow-md active:scale-95 whitespace-nowrap ml-auto">
                ...
                <div className="w-full md:w-1/3 bg-slate-900 border-r border-slate-700 flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-2">
                        {Object.values(players).filter(p => p.name.toLowerCase().includes(searchTerm.toLowerCase())).map(p => (
                            <div 
                                key={p.id} 
                                onClick={() => { play('click'); setSelectedPlayerId(p.id); }}
                        ...
                        <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                            ...
                            <div className="bg-slate-800 p-4 rounded-2xl border border-slate-700">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <button disabled={!parseInputCurrency(amountStr)} onClick={() => { play('click'); executeAction('admin_add_money', selectedPlayer.id, parseInputCurrency(amountStr)); }} className="bg-emerald-600 hover:bg-emerald-500 text-white py-2 rounded-lg font-bold text-xs disabled:opacity-50">CREDITAR (+)</button>
                                    <button disabled={!parseInputCurrency(amountStr)} onClick={() => { play('click'); executeAction('admin_remove_money', selectedPlayer.id, parseInputCurrency(amountStr)); }} className="bg-red-600 hover:bg-red-500 text-white py-2 rounded-lg font-bold text-xs disabled:opacity-50">DEBITAR (-)</button>
                                </div>
                            </div>
                            ...
                            <div className="bg-slate-800 p-4 rounded-2xl border border-slate-700">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {selectedPlayer.isJailed ? (
                                        <button onClick={() => { play('click'); executeAction('admin_unjail', selectedPlayer.id); }} className="bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-lg font-bold text-xs flex items-center justify-center gap-2"><Unlock size={16}/> SOLTAR</button>
                                    ) : (
                                        <button onClick={() => { play('click'); executeAction('admin_jail', selectedPlayer.id); }} className="bg-orange-600 hover:bg-orange-500 text-white py-3 rounded-lg font-bold text-xs flex items-center justify-center gap-2"><Siren size={16}/> PRENDER</button>
                                    )}
                                </div>
                                <div className="grid grid-cols-2 gap-2 mt-2">
                                    <button onClick={() => { play('click'); executeAction('admin_freeze', selectedPlayer.id); }} className="bg-cyan-600 hover:bg-cyan-500 text-white py-3 rounded-lg font-bold text-xs flex items-center justify-center gap-2"><Snowflake size={16}/> CONGELAR</button>
                                    <button onClick={() => { play('click'); if(confirm("Expulsar jogador?")) executeAction('admin_kick', selectedPlayer.id); }} className="bg-red-700 hover:bg-red-600 text-white py-3 rounded-lg font-bold text-xs flex items-center justify-center gap-2"><Ban size={16}/> EXPULSAR</button>
                                </div>
                            </div>
                            ...
                            <div className="bg-slate-800 p-4 rounded-2xl border border-slate-700">
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    {Object.entries(ITEM_NAMES).map(([id, name]) => (
                                        <button key={id} onClick={() => { play('click'); executeAction('admin_add_item', selectedPlayer.id, 0, id); }} className="bg-indigo-600/20 hover:bg-indigo-600 text-indigo-300 hover:text-white py-2 rounded-lg font-bold text-[10px] border border-indigo-500/30 transition">DAR {name.toUpperCase()}</button>
                                    ))}
                                </div>
                            </div>
